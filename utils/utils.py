...
import cv2
import numpy as np
from utils.utils import aspectaware_resize_padding  # 假设原始函数已导入

def preprocess(*image_path, max_size=512, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), is_grayscale=False):
    if is_grayscale:
        # 单通道红外图像加载
        ori_imgs = [cv2.imread(img_path, cv2.IMREAD_GRAYSCALE) for img_path in image_path]
        for img in ori_imgs:
            if img is None:
                raise FileNotFoundError(f"Image {img_path} not found")
        # 添加通道维度 (H, W) -> (H, W, 1)
        ori_imgs = [img[..., np.newaxis] for img in ori_imgs]
        # 使用单通道均值和标准差
        mean = mean[0] if isinstance(mean, (list, tuple)) else mean
        std = std[0] if isinstance(std, (list, tuple)) else std
        # 归一化
        normalized_imgs = [(img / 255.0 - mean) / std for img in ori_imgs]
    else:
        # 原始 RGB 图像加载
        ori_imgs = [cv2.imread(img_path) for img_path in image_path]
        for img, path in zip(ori_imgs, image_path):
            if img is None:
