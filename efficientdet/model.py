...
class EfficientNet(nn.Module):
    def __init__(self, compound_coef, in_channels=3, load_weights=False):
        super(EfficientNet, self).__init__()
        ...
        # 替换 _conv_stem 以支持自定义输入通道数
        self.model._conv_stem = Conv2dStaticSamePadding(
            in_channels, 32, kernel_size=3, stride=2, bias=False, norm='bn'
        )
        
        # 如果加载预训练权重，调整 _conv_stem 的权重
        if load_weights:
            conv_stem_weight = state_dict['_conv_stem.weight']  # 形状 [32, 3, 3, 3]
            if in_channels == 1:
                # 将三通道权重转换为单通道（取平均）
                conv_stem_weight = conv_stem_weight.mean(dim=1, keepdim=True)  # [32, 1, 3, 3]
            state_dict['_conv_stem.weight'] = conv_stem_weight
