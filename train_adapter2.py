...
def train_adapter(source_loader, target_loader, model, adapter, opt, params):
    ...
    for epoch in range(opt.adapter_epochs):
        ...
        for source_data, target_data in zip(source_loader, target_loader):
            try:
                ...
                with torch.no_grad():
                    source_features, _, _, _ = model(source_imgs)  # 提取源域特征
                    target_features, _, _, _ = model(target_imgs)  # 提取目标域特征

                    # 选择最后一层的特征图（假设 P7）
                    source_features = source_features[-1]  # 选择最后一层特征
                    target_features = target_features[-1]  # 选择最后一层特征
                ...
                adapted_target_features = adapter(target_features)  # 适配目标域特征
                loss = coral_loss(source_features, adapted_target_features)  # 计算 CORAL 损失
    ...
    # 保存适配器权重
    torch.save(adapter.state_dict(), os.path.join(opt.saved_path, f'adapter_d{opt.compound_coef}_{epoch+1}.pth'))
...
def train(opt):
    ...
    # 动态计算特征维度
    with torch.no_grad():
        sample_batch = next(iter(source_loader))  # 使用内置 iter 函数
    ...
    # 初始化 FeatureAdapter
    adapter = FeatureAdapter(feature_dim=feature_dim).cuda()
