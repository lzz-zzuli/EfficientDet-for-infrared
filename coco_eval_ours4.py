    ...
    for image_id in tqdm(image_ids, desc="Evaluating images"):
        ...
        with torch.no_grad():
            ...
            if adapter:
                ...
                for layer in layers_to_use:
                    layer_features = features[layer]
                    layer_features = layer_features.contiguous().reshape(layer_features.size(0), -1)
                    source_combined_features.append(layer_features)
                source_combined_features = torch.cat(source_combined_features, dim=1)

                # 保存原始特征
                original_features = source_combined_features.clone()
                # 适配特征
                adapted_features = adapter(source_combined_features)
                ...
                for layer in layers_to_use:
                    layer_features = features[layer]
                    layer_dim = layer_features.contiguous().reshape(layer_features.size(0), -1).size(1)
                    layer_adapted_features = adapted_features[:, start_idx:start_idx+layer_dim].view_as(layer_features)
                    new_features.append(layer_adapted_features)
                    start_idx += layer_dim
                # 替换相应层的特征
                for i, layer in enumerate(layers_to_use):
                    features = list(features)
                    features[layer] = new_features[i]
                ...
                # 计算特征和预测输出的变化
                feature_diff = torch.norm(adapted_features - original_features, p=2)
